...
import {userSignup, createPresignedUrl, checkEmail, checkNickname,} from '../services/signupRequest.js';
...
const sendSignupData = async () => {
    ...
    const { ok, status, body } = await userSignup(props);

    if (!ok) {
        Dialog(
            '회원 가입 실패',
            (body && body.message) || '잠시 뒤 다시 시도해 주세요'
        );
        return;
    }
